...
Now, to do some visualisations, lets calculate the average of the vaccination rates and different expemtion columns and generate new columns storing these values. Note that we could use `summarise()` function to calculate the mean for these columns too, however, that would drop rows after mean calculations on each group. Try using `summarise()` instead of `mutate()` and see how this would be different. As I will need all rows in the data set in one of the plots that I generate later, I do not sue `summarise()`.
